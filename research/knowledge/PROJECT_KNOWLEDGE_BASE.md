...
| :--- | :--- | :--- |
| **Bale OTP Fails (`StartPhoneAuth`)** | Stale `app_version` or missing paired `mt_*` and `language: fa` headers in gRPC-Web request. | Updated `app_version` to `169491` and added full header set in `internal/bale/auth.go`. Stale cookies are no longer injected. |
